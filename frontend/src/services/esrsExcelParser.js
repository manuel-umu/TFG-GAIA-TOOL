...
/**
 * Parsea un ArrayBuffer con el formato EFRAG IG3.
 * Devuelve { standards } — los metadatos de framework/version los aporta el formulario.
 *
 * @param {ArrayBuffer} arrayBuffer
 * @returns {{ standards: Array }}
 */
export function parseEsrsExcel(arrayBuffer) {
    ...
    for (const sheetName of wb.SheetNames) {
        ...
        if (!meta) continue;
        ExcelParser._parseSheet(wb.Sheets[sheetName], sheetName, meta, standardsMap);
    }

    return { standards: Array.from(standardsMap.values()) };
}
...
class ExcelParser {
    ...
    static _parseSheet(ws, sheetName, meta, standardsMap) {
        ...
        // El ESRS 2 esta dividido en dos hojas, ambas van al mismo estándar
    ...
    static _buildDataPoint(row, colMap, officialId) {
        ...
        return {
            ...
            name: ExcelParser._str(row[colMap.name]) || officialId,
    ...
    static _str(value) {
        ...
        const trimmed = String(value).trim().replace(/\s+/g, ' '); // Limpiar espacios dobles o saltos de linea
    ...
    // Columna H, "Conditional" = Si en la celda aparece "conditional" o "alternative" se considera condicional (true),
    // si esta vacio o no contiene esas palabras se considera no condicional (false).
